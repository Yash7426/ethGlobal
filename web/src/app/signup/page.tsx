...
const page = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (signer != undefined) {
      const ccnsLookup: CrossChainNameServiceLookup =
        CrossChainNameServiceLookup__factory.connect(json.ccnsLookup, signer);
      const ccnsRegister: CrossChainNameServiceRegister =
        CrossChainNameServiceRegister__factory.connect(
          json.ccnsRegister,
          signer
        );
      try {
        ...
        <div className="w-full max-w-xl mx-auto p-8">
          ...
          {signer && (
            <form className="flex flex-col gap-4 z-50" onSubmit={handleSubmit}>
              ...
              />
              {message && (
                <p className="text-s text-red-500"> user already exists</p>
              )}
              {!isSubmitted && (
                <m.button
                  initial={{
