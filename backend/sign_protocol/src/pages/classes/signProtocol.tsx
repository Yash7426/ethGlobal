...
import { WhitelistHook__factory,WhitelistHook } from "../../../typechain-types";
...
export default class SignProtocol  {
    ...
    constructor(client: SignProtocolClient) {
         this.client = client;
    }
   ...
   async addAttester() : Promise<String>{   
    const WhitelistHook : WhitelistHook  = WhitelistHook__factory.connect('0x442B7f3595eE078D79bcbdE21A9Bb191f4010De5',this.signer);
    ...
    async createNotaryAttestation(name: string, message: string, schemeId : string) : Promise<string>{
...
async function queryAddress(address: string|undefined):Promise<(PageInfo & { rows: SchemaInfo[]; }) | ErrorResponse | null> {
    try {
